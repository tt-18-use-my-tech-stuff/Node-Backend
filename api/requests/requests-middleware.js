...
const checkRequestExists = (req, res, next) => {
  Request.getById(req.params.request_id)
    .then((request) => {
      if (request) {
        req.request = request;
        next();
...
const checkResponse = async (req, res, next) => {
  ...
  if (response === undefined) {
    ...
    if (req.request.status !== 'pending') {
      next({
        status: 400,
        message: 'Cannot accept a request that is not pending.',
      });
    } else {
      ...
      if (item.renter_id) {
        next({
          status: 400,
          message: 'The item is being rented by another user.',
        });
      } else {
        next();
      }
    ...
    if (req.request.status !== 'pending') {
      next({
        status: 400,
        message: 'Cannot decline a request that is not pending.',
      });
    } else {
      next();
    }
    ...
    if (req.request.status !== 'accepted') {
      next({
        status: 400,
        message: 'Cannot complete a request that is not accepted.',
      });
    } else {
      next();
    }
    ...
    next({
      status: 400,
      message: `response must be "accepted", "declined", or "completed".`,
    });
  }
};
